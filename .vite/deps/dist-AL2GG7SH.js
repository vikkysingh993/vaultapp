...
function parseStructs(signatures) {
  ...
  for (let i4 = 0; i4 < entriesLength; i4++) {
    const [name, parameters] = entries[i4];
...
function ripemd1603(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = ripemd1602(from5(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
function sha2564(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2563(from5(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
...
var AmbiguityError = class extends BaseError4 {
  constructor(x2, y3) {
...
var NotFoundError = class extends BaseError4 {
...
var InvalidSelectorSizeError = class extends BaseError4 {
  constructor({ data }) {
...
function encodeTuple(value, options) {
  ...
  return {
    dynamic,
    encoded: dynamic ? encode3(preparedParameters) : concat3(...preparedParameters.map(({ encoded }) => encoded))
  };
}
...
(function(encodePacked3) {
  function encode11(type6, value, isArray = false) {
    ...
    const intMatch = type6.match(integerRegex3);
    if (intMatch) {
    ...
    const bytesMatch = type6.match(bytesRegex3);
    if (bytesMatch) {
...
var DataSizeTooSmallError = class extends BaseError4 {
...
var ZeroDataError = class extends BaseError4 {
  constructor() {
...
var ArrayLengthMismatchError = class extends BaseError4 {
...
var InvalidArrayError = class extends BaseError4 {
  constructor(value) {
...
function encode5(abiConstructor, options) {
  ...
  return concat3(bytecode, ((_a = abiConstructor.inputs) == null ? void 0 : _a.length) && (args == null ? void 0 : args.length) ? encode4(abiConstructor.inputs, args) : "0x");
}
...
var ArgsMismatchError = class extends BaseError4 {
...
var InputNotFoundError = class extends BaseError4 {
...
var DataMismatchError = class extends BaseError4 {
...
var TopicsMismatchError = class extends BaseError4 {
...
var SelectorTopicMismatchError = class extends BaseError4 {
...
var InvalidStorageKeySizeError = class extends BaseError4 {
...
function readLength(cursor, prefix, offset) {
  ...
  throw new BaseError4("Invalid RLP prefix");
}
...
function fromDerHex(signature) {
  const { r: r4, s: s3 } = secp256k1.Signature.fromDER(from4(signature).slice(2));
  return { r: r4, s: s3 };
}
...
function fromString5(value) {
  return from17(fromString4(value));
}
...
function fromString6(value, options = {}) {
  return fromBytes6(fromString4(value), options);
}
  ...
  function inner(node) {
    ...
    while (level.length > 1) {
      ...
      for (let i4 = 0; i4 < level.length; i4 += 2)
        level_.push(hash3(concat4(level[i4], level[i4 + 1])));
      level = level_;
    }
...
var EmptyBlobError = class extends BaseError4 {
  constructor() {
...
var EmptyBlobVersionedHashesError = class extends BaseError4 {
  constructor() {
...
var InvalidVersionedHashSizeError = class extends BaseError4 {
...
var InvalidVersionedHashVersionError = class extends BaseError4 {
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b2, from33, to3) => bytesToNumberBE(b2.slice(from33, to3));
...
function getPublicKey(options) {
  ...
  const { px, py, pz } = group.ProjectivePoint.fromPrivateKey(from4(privateKey).slice(2));
...
function sign3(options) {
  ...
  const payloadPoint = payloadGroup.hashToCurve(from5(payload), suite ? { DST: fromString4(suite) } : void 0);
...
function verify(options) {
  ...
  const payloadPoint = group.hashToCurve(from5(payload), suite ? { DST: fromString4(suite) } : void 0);
...
function from20(options) {
  if (options.salt)
    return fromCreate2(options);
  return fromCreate(options);
}
function fromCreate(options) {
  const from33 = fromHex2(from7(options.from));
  let nonce = fromNumber3(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from7(`0x${keccak2563(fromBytes3([from33, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  const from33 = fromHex2(from7(options.from));
  const salt = padLeft3(validate3(options.salt) ? options.salt : fromHex2(options.salt), 32);
  const bytecodeHash = (() => {
  ...
  return from7(slice3(keccak2563(concat4(fromHex2("0xff"), from33, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function namehash(name) {
  ...
  for (let i4 = labels.length - 1; i4 >= 0; i4 -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex2(hashFromEncodedLabel) : keccak2563(fromString4(labels[i4]), { as: "Bytes" });
...
function sign4(options) {
  ...
  const { r: r4, s: s3, recovery } = secp256k1.sign(from5(payload), from5(privateKey), {
...
function verify2(options) {
  ...
  return secp256k1.verify(signature, from5(payload), toBytes3(publicKey), ...hash9 ? [{ prehash: true, lowS: true }] : []);
}
...
function fromSeed(seed, options = {}) {
  ...
  const key = HDKey.fromMasterSeed(from5(seed), versions);
  return fromScure(key);
}
...
function from21(response, options = {}) {
...
function from22(provider, options = {}) {
  var _a, _b;
...
var IsUndefinedError = class extends BaseError4 {
  constructor() {
...
var TimeoutError = class extends BaseError4 {
  constructor() {
...
var HttpError = class extends BaseError4 {
...
var MalformedResponseError = class extends BaseError4 {
...
var InvalidMessageFieldError = class extends BaseError4 {
  constructor(parameters) {
...
function from25(value, decimals = 0) {
...
function fromEther(ether, unit = "wei") {
  return from25(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from25(gwei, exponents.gwei - exponents[unit]);
}
...
// node_modules/ox/_esm/core/TransactionEnvelope.js
var FeeCapTooHighError = class extends BaseError4 {
  constructor({ feeCap } = {}) {
...
var GasPriceTooHighError = class extends BaseError4 {
...
var InvalidChainIdError = class extends BaseError4 {
  constructor({ chainId }) {
...
var InvalidSerializedError = class extends BaseError4 {
...
var TipAboveFeeCapError = class extends BaseError4 {
...
function assert12(value) {
  ...
  const validateData = (struct, data) => {
    for (const param of struct) {
      const { name, type: type6 } = param;
      const value2 = data[name];
      const integerMatch = type6.match(integerRegex3);
      if (integerMatch && (typeof value2 === "number" || typeof value2 === "bigint")) {
        const [, base3, size_] = integerMatch;
        fromNumber2(value2, {
          signed: base3 === "int",
...
var BytesSizeMismatchError3 = class extends BaseError4 {
  constructor({ expectedSize, givenSize }) {
    super(`Expected bytes${expectedSize}, got bytes${givenSize}.`);
    Object.defineProperty(this, "name", {
      enumerable: true,
...
var InvalidDomainError2 = class extends BaseError4 {
  constructor({ domain }) {
...
var InvalidPrimaryTypeError2 = class extends BaseError4 {
  constructor({ primaryType, types: types2 }) {
    super(`Invalid primary type \`${primaryType}\` must be one of \`${JSON.stringify(Object.keys(types2))}\`.`, {
      metaMessages: ["Check that the primary type is a key in `types`."]
    });
...
var InvalidStructTypeError2 = class extends BaseError4 {
  constructor({ type: type6 }) {
    super(`Struct type "${type6}" is invalid.`, {
      metaMessages: ["Struct type must not be a Solidity type."]
    });
...
function getClientDataJSON(options) {
  ...
  return JSON.stringify({
    type: "webauthn.get",
    challenge: fromHex7(challenge, { url: true, pad: false }),
    origin,
    crossOrigin,
    ...extraClientData
  });
...
async function verify5(options) {
  ...
  return await globalThis.crypto.subtle.verify({
    name: "ECDSA",
    hash: "SHA-256"
  }, publicKey, concat4(fromNumber3(signature.r), fromNumber3(signature.s)), from5(payload));
}
...
function createWalletSendCallsRequest({ calls, from: from33, chainId, capabilities }) {
...
function from32(abiError, options = {}) {
  return from(abiError, options);
}
var solidityError2 = from32({
  inputs: [
    {
      name: "message",
      type: "string"
  ...
  const subAccountCallData = encodeFunctionData({
    ...
    args: [
      originalSendCallsParams.calls.map((call2) => ({
        target: call2.to,
        value: hexToBigInt(call2.value ?? "0x0"),
        data: call2.data ?? "0x"
      }))
    ]
  });
...
var getHashFn = async ({ permission, chainId }) => {
  ...
  const hash9 = await readContract(client, {
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=dist-AL2GG7SH.js.map
